...
import jatymon.babelprojects.abp.timers.ConnectionFailureTimer;
...
public class FaultyDispatcherProtocol extends DispatcherProtocol {
    ...
    public void init(final Properties props) throws HandlerRegistrationException, IOException {
        ...
        registerTimerHandler(ConnectionFailureTimer.ID, this::uponFailureTimer);
        setupTimer(new ConnectionFailureTimer(), getRandomFaultDelay());
    }

    @Override
    protected void uponAckMessage(AckMessage message, final Host sender, final short sourceProto, final int channelId) {
        switch (currentFaultType) {
            ...
            case MessageTinker -> {
                if (injectMessageTinker(message)) {
                    message = new AckMessage((byte)(message.getBit() ^ 1));
                }
            }
        }
        super.uponAckMessage(message, sender, sourceProto, channelId);
    }

    @Override
    protected void uponBitMessage(BitMessage message, final Host sender, final short sourceProto, final int channelId) {
        switch (currentFaultType) {
            case MessageDrop -> {
                if (injectMessageDrop(message)) {
                    return;
            ...
            case MessageTinker -> {
                if (injectMessageTinker(message)) {
                    message = new BitMessage((byte)(message.getBit() ^ 1));
                }
            }
        }
        super.uponBitMessage(message, sender, sourceProto, channelId);
    }

    private void uponFailureTimer(final ConnectionFailureTimer timer, final long timerId) {
        if (enabledFaults.isEmpty()) {
            return;
        } else if (random.nextFloat() <= faultProbability) {
