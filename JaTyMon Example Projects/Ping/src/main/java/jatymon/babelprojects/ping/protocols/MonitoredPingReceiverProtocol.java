...
public class MonitoredPingReceiverProtocol extends PingReceiverProtocol {
  private static final String TYPESTATE_NAME = "PingReceiver";
  ...
  public MonitoredPingReceiverProtocol() {
  ...
  public MonitoredPingReceiverProtocol(final JaTyMonLogger logger, final double confidenceLevel) {
  ...
  @Override
  public void sendEchoReply(Host arg0) {
    final States oldState = currentState;
    switch (currentState) {
      case State1:
        try {
          currentState = States.State1;
          super.sendEchoReply(arg0);
        } catch(final Exception e) {
          currentState = oldState;
          logger.log(new InternalExceptionLog(TYPESTATE_NAME, e.getMessage()));
        }
      ...
      default:
        try {
          logger.log(new IllegalActionLog(TYPESTATE_NAME, currentState.toString(), "sendEchoReply", ActionType.Output));
          super.sendEchoReply(arg0);
        } catch (final Exception e) {
          currentState = oldState;
          logger.log(new InternalExceptionLog(TYPESTATE_NAME, e.getMessage()));
