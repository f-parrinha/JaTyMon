...
public class MonitoredPingProtocol extends PingProtocol {
  private static final String TYPESTATE_NAME = "Ping";
  ...
  public MonitoredPingProtocol(final JaTyMonLogger logger, final double confidenceLevel) {
  ...
  @Override
  public void uponReceivedEchoReply(ReceivedEchoReply arg0, short arg1) {
    final States oldState = currentState;
    switch (currentState) {
      case State1:
        validateRatio(new ActionId("State1", "uponReceivedEchoReply", ActionType.Input), 0.5);
        try {
          currentState = States.State1;
          super.uponReceivedEchoReply(arg0, arg1);
        } catch(final Exception e) {
          currentState = oldState;
          logger.log(new InternalExceptionLog(TYPESTATE_NAME, e.getMessage()));
        }
      ...
      default:
        try {
          logger.log(new IllegalActionLog(TYPESTATE_NAME, currentState.toString(), "uponReceivedEchoReply", ActionType.Input));
          super.uponReceivedEchoReply(arg0, arg1);
        } catch (final Exception e) {
          currentState = oldState;
          logger.log(new InternalExceptionLog(TYPESTATE_NAME, e.getMessage()));
  ...
  @Override
  public void uponReceivedEchoRequest(ReceivedEchoRequest arg0, short arg1) {
    final States oldState = currentState;
    switch (currentState) {
      case State1:
        validateRatio(new ActionId("State1", "uponReceivedEchoRequest", ActionType.Input), 0.5);
        try {
          currentState = States.State1;
          super.uponReceivedEchoRequest(arg0, arg1);
        } catch(final Exception e) {
          currentState = oldState;
          logger.log(new InternalExceptionLog(TYPESTATE_NAME, e.getMessage()));
        }
      ...
      default:
        try {
          logger.log(new IllegalActionLog(TYPESTATE_NAME, currentState.toString(), "uponReceivedEchoRequest", ActionType.Input));
          super.uponReceivedEchoRequest(arg0, arg1);
        } catch (final Exception e) {
          currentState = oldState;
          logger.log(new InternalExceptionLog(TYPESTATE_NAME, e.getMessage()));
